...
describe('Umbra', () => {
  ...
  before(async () => {
    ...
    await this.token.mint(payer2, totalTokenAmount);
  });
  ...
  it('should have correct values initialized', async () => {
    ...
    expect(tokenBalance.toString()).to.equal(totalTokenAmount); // TODO: Add these
  });
  ...
  it('should allow the toll collector to move tolls to toll receiver', async () => {
    const toll = await this.umbra.toll();
    const expectedCollection = toll.mul(new BN('5'));
